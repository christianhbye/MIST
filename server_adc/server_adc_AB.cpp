...
#define DATAPOINTS 16384 //number of datapoints per time window (16384 = 2^14) per channel
...
/*
To compile: g++ server_adc_AB.cpp -o server_adc_AB
To run: ./server_adc_AB
...
//all quantities are multiplied by 2 to reflect that are 2 channels
unsigned short int buff[2*DATAPOINTS]; //array to be sent
unsigned short int data_sent[2*TW*DATAPOINTS]; //array to store sent data
unsigned short int data_back[2*TW * DATAPOINTS]; //array to store data received for accuracy check
unsigned short int data_back_tot[2*TW * DATAPOINTS]; //for each read(), the content of "data_back" is copied here before "data_back" is overwritten by a new call of read()
...
void func(int sockfd) //function for sending and receiving data
{
	...
	for (;;) { //infinite loop for sending data
		...
		if (input == 0) {
			...
			int bytes2copy = 2 * DATAPOINTS * sizeof(unsigned short int); //bytes of data in one time window
	...
	//			printf("Bytes written: %d \n", x);
		...
		//	printf("Data sent: /n %s /n", data_sent);
			...
			if (acc_check == 1)
			{
				...
				for (int k = 0; k < 2*TW*DATAPOINTS; k++)
				{
//					printf("Back: %hu \n", data_back_tot[k]);
//					printf("Sent: %hu \n", data_sent[k]);
