...
#define DATAPOINTS 16384 //number of datapoints per timewindow (needs to match the value of DATAPOINTS in the server program)
//#define DATAPOINTS 2*16384 //to receive data captured from both channels of the ADC (will be twice the number of DATAPOINTS in the server program)
...
/*
Note: This version of the client program is made to receive data from one channnel only of the Terasic ADC-Soc. To receive data from
two channels, change DATAPOINTS in the header by cooomenting out the first line where it is mentioned and uncommenting the line below.
